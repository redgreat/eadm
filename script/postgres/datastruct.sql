...
-- 佳明运动数据
drop sequence if exists sa cascade;
create sequence sa
start 1
increment by 1
maxvalue 9999999999
...
drop table if exists sp_activity cascade;
create table sp_activity (
  id char(12) not null default ('sa' || lpad((nextval('sa')::varchar), 10, '0')),
  userid char(12) not null,
  actname varchar(250),
  actmemo varchar(1000),
  ...
  elevationgain int,
  elevationloss int,
  avgspeed numeric(18,2),
  maxspeed numeric(18,2),
  avgheartbeat smallint,
  maxheartbeat smallint,
  avgpower int,
  maxpower int,
  avgcadence int,
  maxcadence int,
  ...
  garminactid varchar(50),
  ispublic boolean not null default false,
  sharetoken varchar(64),
  hidemap boolean not null default false,
...
alter table sp_activity owner to user_eadm;
alter table sp_activity drop constraint if exists pk_spactivity_id cascade;
alter table sp_activity add constraint pk_spactivity_id primary key (id);

alter table sp_activity drop constraint if exists fk_userid_user_id cascade;
alter table sp_activity add constraint fk_userid_user_id foreign key (userid)
    references eadm_user (id) on delete restrict on update restrict;

alter table sp_activity drop constraint if exists uni_spactivity_garminactid cascade;
alter table sp_activity add constraint uni_spactivity_garminactid unique (garminactid);
alter table sp_activity drop constraint if exists uni_spactivity_sharetoken cascade;
alter table sp_activity add constraint uni_spactivity_sharetoken unique (sharetoken);

drop index if exists non_spactivity_userid;
create index non_spactivity_userid on sp_activity using btree (userid asc nulls last);
drop index if exists non_spactivity_starttime;
create index non_spactivity_starttime on sp_activity using btree (starttime desc nulls last);
drop index if exists non_spactivity_ispublic;
create index non_spactivity_ispublic on sp_activity using btree (ispublic asc nulls last);

comment on column sp_activity.id is '自定义主键(sa)';
comment on column sp_activity.userid is '用户id(eadm_user.id)';
comment on column sp_activity.actname is '活动名称';
comment on column sp_activity.actmemo is '活动备注';
comment on column sp_activity.acttype is '运动类型(1跑步2骑行3游泳4健身5徒步99其他)';
comment on column sp_activity.distance is '距离(米)';
comment on column sp_activity.starttime is '开始时间';
comment on column sp_activity.endtime is '结束时间';
comment on column sp_activity.timespan is '活动时长(秒)';
comment on column sp_activity.elevationgain is '累计爬升(米)';
comment on column sp_activity.elevationloss is '累计下降(米)';
comment on column sp_activity.avgspeed is '平均速度(m/s)';
comment on column sp_activity.maxspeed is '最大速度(m/s)';
comment on column sp_activity.avgheartbeat is '平均心率(bpm)';
comment on column sp_activity.maxheartbeat is '最大心率(bpm)';
comment on column sp_activity.avgpower is '平均功率(瓦)';
comment on column sp_activity.maxpower is '最大功率(瓦)';
comment on column sp_activity.avgcadence is '平均步频/踏频';
comment on column sp_activity.maxcadence is '最大步频/踏频';
comment on column sp_activity.calorie is '卡路里(千卡)';
comment on column sp_activity.city is '城市';
comment on column sp_activity.country is '国家';
comment on column sp_activity.timezone is '时区';
comment on column sp_activity.garminactid is 'Garmin活动Id';
comment on column sp_activity.ispublic is '是否公开分享(0否1是)';
comment on column sp_activity.sharetoken is '公开分享唯一token';
comment on column sp_activity.hidemap is '是否隐藏地图(0否1是)';
comment on column sp_activity.hidestats is '是否隐藏统计(0否1是)';
comment on column sp_activity.hidelocation is '是否隐藏位置(0否1是)';
comment on column sp_activity.createduser is '创建人';
comment on column sp_activity.createdat is '创建时间';
comment on column sp_activity.updateduser is '更新人';
comment on column sp_activity.updatedat is '更新时间';
comment on column sp_activity.deleteduser is '删除人';
comment on column sp_activity.deletedat is '删除时间';
comment on column sp_activity.deleted is '是否删除(0否1是)';
comment on table sp_activity is '运动域_活动信息表';

drop trigger if exists activity_lastupdate on sp_activity cascade;
create or replace trigger activity_lastupdate
before update on sp_activity
for each row
execute function lastupdate();

drop table if exists sp_stream cascade;
create table sp_stream (
  id serial,
  actid char(12) not null,
  streamtype varchar(50) not null,
  streamjson json not null,
  createduser varchar(50),
  createdat timestamptz default current_timestamp,
  updateduser varchar(50),
  updatedat timestamptz default current_timestamp,
...
alter table sp_stream owner to user_eadm;
alter table sp_stream drop constraint if exists pk_spstream_id cascade;
alter table sp_stream add constraint pk_spstream_id primary key (id);

alter table sp_stream drop constraint if exists fk_actid_spactivity_id cascade;
alter table sp_stream add constraint fk_actid_spactivity_id foreign key (actid)
    references sp_activity (id) on delete restrict on update restrict;

drop index if exists non_spstream_actid;
create index non_spstream_actid on sp_stream using btree (actid asc nulls last);
drop index if exists non_spstream_actid_streamtype;
create index non_spstream_actid_streamtype on sp_stream using btree (actid asc nulls last, streamtype asc nulls last);

comment on column sp_stream.id is '自增主键';
comment on column sp_stream.actid is '活动id(sp_activity.id)';
comment on column sp_stream.streamtype is '数据类型(altitude/heartbeat/cadence/power/distance/time/latlng)';
comment on column sp_stream.streamjson is '数据json';
comment on column sp_stream.createduser is '创建人';
comment on column sp_stream.createdat is '创建时间';
comment on column sp_stream.updateduser is '更新人';
comment on column sp_stream.updatedat is '更新时间';
comment on column sp_stream.deleteduser is '删除人';
comment on column sp_stream.deletedat is '删除时间';
comment on column sp_stream.deleted is '是否删除(0否1是)';
comment on table sp_stream is '运动域_活动数据流表';

drop trigger if exists stream_lastupdate on sp_stream cascade;
create or replace trigger stream_lastupdate
before update on sp_stream
for each row
execute function lastupdate();

drop sequence if exists gi cascade;
create sequence gi
start 1
increment by 1
maxvalue 9999999999
...
drop table if exists sp_garminconf cascade;
create table sp_garminconf (
  id char(12) not null default ('gi' || lpad((nextval('gi')::varchar), 10, '0')),
  userid char(12) not null,
  garminemail varchar(250) not null,
  oauth1token json,
  oauth2token json,
  lastsynctime timestamptz,
  syncenable boolean not null default true,
  autosync boolean not null default true,
  syncdays smallint not null default 30,
  createduser varchar(50),
  createdat timestamptz default current_timestamp,
  updateduser varchar(50),
  updatedat timestamptz default current_timestamp,
...
alter table sp_garminconf owner to user_eadm;
alter table sp_garminconf drop constraint if exists pk_garminconf_id cascade;
alter table sp_garminconf add constraint pk_garminconf_id primary key (id);

alter table sp_garminconf drop constraint if exists fk_userid_user_id cascade;
alter table sp_garminconf add constraint fk_userid_user_id foreign key (userid)
    references eadm_user (id) on delete restrict on update restrict;
alter table sp_garminconf drop constraint if exists uni_garminconf_userid cascade;
alter table sp_garminconf add constraint uni_garminconf_userid unique (userid);

drop index if exists non_garminconf_userid;
create index non_garminconf_userid on sp_garminconf using btree (userid asc nulls last);
drop index if exists non_garminconf_syncenable;
create index non_garminconf_syncenable on sp_garminconf using btree (syncenable asc nulls last);

comment on column sp_garminconf.id is '自定义主键(gi)';
comment on column sp_garminconf.userid is '用户id(eadm_user.id)';
comment on column sp_garminconf.garminemail is 'Garmin账号邮箱';
comment on column sp_garminconf.oauth1token is 'OAuth1认证token(json加密存储)';
comment on column sp_garminconf.oauth2token is 'OAuth2认证token(json加密存储)';
comment on column sp_garminconf.lastsynctime is '最近同步时间';
comment on column sp_garminconf.syncenable is '是否启用同步(0否1是)';
comment on column sp_garminconf.autosync is '是否启用自动同步(0否1是)';
comment on column sp_garminconf.syncdays is '每次同步近N天';
comment on column sp_garminconf.createduser is '创建人';
comment on column sp_garminconf.createdat is '创建时间';
comment on column sp_garminconf.updateduser is '更新人';
comment on column sp_garminconf.updatedat is '更新时间';
comment on column sp_garminconf.deleteduser is '删除人';
comment on column sp_garminconf.deletedat is '删除时间';
comment on column sp_garminconf.deleted is '是否删除(0否1是)';
comment on table sp_garminconf is '运动域_Garmin集成配置表';

drop trigger if exists garminconf_lastupdate on sp_garminconf cascade;
create or replace trigger garminconf_lastupdate
before update on sp_garminconf
for each row
execute function lastupdate();

drop table if exists sp_garminlog cascade;
create table sp_garminlog (
  id serial,
  userid char(12) not null,
  starttime timestamptz not null,
  endtime timestamptz,
...
alter table sp_garminlog owner to user_eadm;
alter table sp_garminlog drop constraint if exists pk_garminlog_id cascade;
alter table sp_garminlog add constraint pk_garminlog_id primary key (id);

alter table sp_garminlog drop constraint if exists fk_userid_user_id cascade;
alter table sp_garminlog add constraint fk_userid_user_id foreign key (userid)
    references eadm_user (id) on delete restrict on update restrict;

drop index if exists non_garminlog_userid;
create index non_garminlog_userid on sp_garminlog using btree (userid asc nulls last);
drop index if exists non_garminlog_starttime;
create index non_garminlog_starttime on sp_garminlog using btree (starttime desc nulls last);
drop index if exists non_garminlog_syncstatus;
create index non_garminlog_syncstatus on sp_garminlog using btree (syncstatus asc nulls last);

comment on column sp_garminlog.id is '自增主键';
comment on column sp_garminlog.userid is '用户id(eadm_user.id)';
comment on column sp_garminlog.starttime is '同步开始时间';
comment on column sp_garminlog.endtime is '同步结束时间';
comment on column sp_garminlog.synccount is '本次同步总活动数';
comment on column sp_garminlog.newcount is '新增活动数';
comment on column sp_garminlog.updcount is '更新活动数';
comment on column sp_garminlog.syncstatus is '同步状态(running/success/failed/partial)';
