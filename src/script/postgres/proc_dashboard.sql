...
-- comment : proc for PG -- PostgreSQL 16.1 (Debian 16.1-1.pgdg120+1) on x86_64-pc-linux-gnu, compiled by gcc (Debian 12.2.0-14) 12.2.0, 64-bit

create or replace procedure public.proc_dashboard(in indate date)
    language plpgsql
AS $procedure$
    -- author: wangcw
...
begin
    ...
    select 1, 2, 'wangcw', nullif(round(avg(heartbeat::int)),0), indate
    from lc_watchdaily
    ...
    from (
             select max(steps::int) as maxsteps
    ...
    from (
             select sum(sleepminute::int) as sumsleepminute
             from lc_watchdaily
             where ptime >= indate - interval '7 days'
               and ptime < indate
    ...
    -- 年度平均心率
    insert into eadm_dashboard(datatype, datetype, loginname, datavalue, checkdate)
    select 1, 4, 'wangcw', nullif(round(avg(heartbeat::int)),0), indate
    from lc_watchdaily
