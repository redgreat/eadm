...
add(#{
    ...
    json := #{
        <<"deviceNo">> := DeviceNo, <<"imei">> := Imei, <<"simNo">> := SimNo, <<"remark">> := Remark
    }
}) ->
    try
        % 检查设备号是否已存在
                        ...
                        {json, [
                            #{
                                <<"Alert">> => unicode:characters_to_binary(
                                    "设备" ++ StatusText ++ "成功！", utf8
                                )
                            }
                        ]};
                    {error, _Reason} ->
                        {json, [
                            #{<<"Alert">> => unicode:characters_to_binary("设备状态更新失败：数据库操作错误", utf8)}
                        ]}
                end;
            [] ->
                % 设备不存在
...
toggle_status(#{
    auth_data := #{<<"permission">> := #{<<"device">> := #{<<"devedit">> := false}}}, json := _
}) ->
...
assign(#{
    ...
    json := #{
        <<"deviceNo">> := DeviceNo, <<"userId">> := UserId, <<"userLoginName">> := UserLoginName
    }
}) ->
    try
        % 检查设备是否已分配给该用户
