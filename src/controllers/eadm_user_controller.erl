                ...
                true ->
                    ...
                    try
                        ...
                        _:Error ->
                            ...
                            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户密码修改失败！")},
                            {json, [Alert]}
                    end;
                2 ->
...
reset(#{auth_data := #{<<"authed">> := true, <<"loginname">> := LoginName,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户密码重置失败！")},
            {json, [Alert]}
    end;
...
disable(#{auth_data := #{<<"authed">> := true, <<"loginname">> := LoginName,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户操作失败！")},
            {json, [Alert]}
    end;
...
delete(#{auth_data := #{<<"authed">> := true, <<"loginname">> := LoginName,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户删除失败！")},
            {json, [Alert]}
    end;
...
userrole(#{auth_data := #{<<"authed">> := true,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户角色查询失败!")},
            {json, [Alert]}
    end;
...
userroleadd(#{auth_data := #{<<"authed">> := true, <<"loginname">> := LoginName,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户角色新增失败!")},
            {json, [Alert]}
    end;
...
userroledel(#{auth_data := #{<<"authed">> := true, <<"loginname">> := LoginName,
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户角色删除失败!")},
            {json, [Alert]}
    end;
...
validate_password(PassWordBin) when is_binary(PassWordBin) ->
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("密码验证失败！")},
            {json, [Alert]}
    end;
validate_password(_) ->
    {error, "密码格式错误！"}.
...
validate_addloginname(LoginName) ->
    ...
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("用户名验证失败！")},
            {json, [Alert]}
    end.
...
get_permission(LoginName) ->
    try
        ...
        _:Error ->
            ...
            Alert = #{<<"Alert">> => unicode:characters_to_binary("权限获取失败!")},
            {json, [Alert]}
    end.
